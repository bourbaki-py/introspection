...
@lazy_imports(
    import_("pathlib").as_("foolib"), from_("uuid").import_("UUID", uuid4="uuidfour")
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            __import__("typing_inspect").get_args,
            site_packages,
        ),
    ],
)
...
@pytest.mark.parametrize("path", ["re.asdfasdf", "collections.asdfasdf"])
def test_import_object_raises_attribute_error(path):
    with pytest.raises(AttributeError):
        obj = import_object(path)


@pytest.mark.parametrize("path", ["asdfasdf", "asdf.asdf"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Tuple[int,...]", Tuple[int, ...]),
        (
            "Dict[Tuple[int, str, bool], Sequence[complex]]",
            Dict[Tuple[int, str, bool], Sequence[complex]],
        ),
...
@pytest.mark.parametrize("path", ["-foo.bar", "{'foo': 'bar'}", "foo.bar{baz}"])
def test_import_type_raises_syntax_error(path):
    with pytest.raises(SyntaxError):
        type_ = import_type(path)


@pytest.mark.parametrize("path", ["re.I", "pathlib", "int.__str__"])
...
@pytest.mark.parametrize(
    "path", ["pathlib.Path", "collections.defaultdict", "int", "asdfasdg.asdgasdg"]
)
